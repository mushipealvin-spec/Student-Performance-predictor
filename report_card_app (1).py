...
class ReportCardApp(tk.Tk):
    ...
    def _on_canvas_resize(self, event):
        width = max(event.width - 30, 1100)
        self.canvas.coords(self.sheet_window, event.width / 2, 0)
        self.canvas.itemconfig(self.sheet_window, width=width)
    ...
    def _build_selection_bar(self):
        bar = tk.Frame(self.sheet, bg=SOFT, pady=20, padx=24)
        bar.pack(fill="x")
        for c in range(5):
            bar.columnconfigure(c, weight=1)

        # Student Name
        tk.Label(bar, text="STUDENT NAME", font=("Segoe UI", 10, "bold"),
                 bg=SOFT, fg=MUTED).grid(row=0, column=0, sticky="w", padx=8)
        self.student_name_var = tk.StringVar()
        tk.Entry(bar, textvariable=self.student_name_var, font=("Segoe UI", 12)
                 ).grid(row=1, column=0, sticky="ew", padx=8, pady=(4, 0), ipady=4)

        # Enrollment Number
        tk.Label(bar, text="ENROLLMENT NUMBER", font=("Segoe UI", 10, "bold"),
                 bg=SOFT, fg=MUTED).grid(row=0, column=1, sticky="w", padx=8)
        self.enrollment_var = tk.StringVar()
        tk.Entry(bar, textvariable=self.enrollment_var, font=("Segoe UI", 12)
                 ).grid(row=1, column=1, sticky="ew", padx=8, pady=(4, 0), ipady=4)

        # Programme / Department dropdown
        tk.Label(bar, text="PROGRAMME / DEPARTMENT", font=("Segoe UI", 10, "bold"),
                 bg=SOFT, fg=MUTED).grid(row=0, column=2, sticky="w", padx=8)
        ...
        programme_box.grid(row=1, column=2, sticky="ew", padx=8, pady=(4, 0), ipady=4)
        programme_box.bind("<<ComboboxSelected>>", lambda e: self.render_courses())

        # Semester dropdown
        tk.Label(bar, text="SEMESTER", font=("Segoe UI", 10, "bold"),
                 bg=SOFT, fg=MUTED).grid(row=0, column=3, sticky="w", padx=8)
        self.semester_var = tk.StringVar()
        semester_box = ttk.Combobox(bar, textvariable=self.semester_var,
                                     values=[str(i) for i in range(1, 9)],
                                     state="readonly", font=("Segoe UI", 12))
        semester_box.grid(row=1, column=3, sticky="ew", padx=8, pady=(4, 0), ipady=4)
        semester_box.bind("<<ComboboxSelected>>", lambda e: self.render_courses())

        # Show Result button
        show_btn = tk.Button(bar, text="Show Result", font=("Segoe UI", 12, "bold"),
                              bg=BRAND, fg="white", relief="flat", padx=18, pady=10,
                              activebackground=BRAND_DARK, activeforeground="white",
                              command=self.calculate_result)
        show_btn.grid(row=1, column=4, sticky="ew", padx=8, pady=(4, 0))

    # ---------------- student info line ----------------
    def _build_student_line(self):
        line = tk.Frame(self.sheet, bg=SURFACE, padx=24, pady=18)
        line.pack(fill="x")
        for c in range(4):
    ...
    def _info_box(self, parent, label, col):
        ...
        tk.Label(box, text=label, font=("Segoe UI", 9, "bold"), bg=SURFACE,
                 fg=MUTED, anchor="w").pack(fill="x", padx=12, pady=(10, 0))
        value = tk.Label(box, text="-", font=("Segoe UI", 15, "bold"), bg=SURFACE,
                          fg=INK, anchor="w")
    ...
    def _build_table_section(self):
        ...
        for c, w in enumerate(widths):
            self.table_frame.columnconfigure(c, weight=w, uniform="cols")
        for c, (h, w) in enumerate(zip(headers, widths)):
            tk.Label(self.table_frame, text=h, font=("Segoe UI", 11, "bold"), bg="#e9eef5",
                     fg="#344054", borderwidth=1, relief="solid"
                     ).grid(row=0, column=c, sticky="nsew", ipady=8)

        self.body_start_row = 1  # rows will be inserted starting here

    # ---------------- summary ----------------
    def _build_summary_section(self):
        wrap = tk.Frame(self.sheet, bg=SURFACE, padx=24, pady=18)
        wrap.pack(fill="x")
        for c in range(4):
    ...
    def _summary_box(self, parent, label, col, badge=False):
        ...
        box.grid(row=0, column=col, sticky="ew", padx=8)
        tk.Label(box, text=label, font=("Segoe UI", 9, "bold"), bg=SOFT,
                 fg=MUTED, anchor="w").pack(fill="x", padx=12, pady=(10, 0))
        if badge:
            value = tk.Label(box, text="-", font=("Segoe UI", 13, "bold"), bg=PASS_C,
                              fg="white", anchor="center", padx=10, pady=6)
            value.pack(padx=12, pady=(6, 12), anchor="w")
        else:
            value = tk.Label(box, text="-", font=("Segoe UI", 18, "bold"), bg=SOFT,
                              fg=INK, anchor="w")
            value.pack(fill="x", padx=12, pady=(0, 12))
        return value
    ...
    def _build_prediction_section(self):
        ...
        box.pack(fill="x")
        tk.Label(box, text="Next Semester Prediction", font=("Segoe UI", 15, "bold"),
                 bg=SURFACE, fg=BRAND).pack(anchor="w", padx=16, pady=(12, 6))
        self.prediction_label = tk.Label(box, text="-", font=("Segoe UI", 14, "bold"),
                                          bg=SURFACE, fg=INK, anchor="w", justify="left")
        self.prediction_label.pack(anchor="w", padx=16)
        self.prediction_basis = tk.Label(box, text="-", font=("Segoe UI", 10), bg=SURFACE,
                                          fg=MUTED, anchor="w", justify="left", wraplength=1300)
        self.prediction_basis.pack(anchor="w", padx=16, pady=(4, 14))

    # ---------------- chart ----------------
    def _build_chart_section(self):
        wrap = tk.Frame(self.sheet, bg=SURFACE, padx=24, pady=12)
        wrap.pack(fill="x")
        tk.Label(wrap, text="Course-wise Performance & Prediction Chart",
                 font=("Segoe UI", 15, "bold"), bg=SURFACE, fg=BRAND
                 ).pack(anchor="w", pady=(0, 10))
        self.chart_holder = tk.Frame(wrap, bg=SURFACE, height=420)
    ...
    def render_courses(self):
        ...
        for r, row in enumerate(rows, start=self.body_start_row):
            ...
            tk.Label(self.table_frame, text=row["code"], bg=bg, fg=INK, font=cell_font,
                     borderwidth=1, relief="solid").grid(row=r, column=0, sticky="nsew", ipady=10)
            tk.Label(self.table_frame, text=row["name"], bg=bg, fg=INK, font=cell_font,
                     anchor="w", borderwidth=1, relief="solid"
                     ).grid(row=r, column=1, sticky="nsew", ipady=10, ipadx=6)
            tk.Label(self.table_frame, text=str(row["externalMax"]), bg=bg, fg=INK, font=cell_font,
                     borderwidth=1, relief="solid").grid(row=r, column=2, sticky="nsew", ipady=10)

            ext_var = tk.StringVar(value=str(row["externalObtained"]))
            tk.Entry(self.table_frame, textvariable=ext_var, font=cell_font, justify="center"
                     ).grid(row=r, column=3, sticky="nsew", ipady=8)

            tk.Label(self.table_frame, text=str(row["internalMax"]), bg=bg, fg=INK, font=cell_font,
                     borderwidth=1, relief="solid").grid(row=r, column=4, sticky="nsew", ipady=10)

            int_var = tk.StringVar(value=str(row["internalObtained"]))
            tk.Entry(self.table_frame, textvariable=int_var, font=cell_font, justify="center"
                     ).grid(row=r, column=5, sticky="nsew", ipady=8)

            tot_max_lbl = tk.Label(self.table_frame, text="-", bg=bg, fg=INK, font=cell_font,
                                    borderwidth=1, relief="solid")
            tot_max_lbl.grid(row=r, column=6, sticky="nsew", ipady=10)
            tot_obt_lbl = tk.Label(self.table_frame, text="-", bg=bg, fg=INK, font=cell_font,
                                    borderwidth=1, relief="solid")
            tot_obt_lbl.grid(row=r, column=7, sticky="nsew", ipady=10)
            status_lbl = tk.Label(self.table_frame, text="-", bg=bg, fg="white",
                                   font=("Segoe UI", 11, "bold"),
                                   borderwidth=1, relief="solid")
            status_lbl.grid(row=r, column=8, sticky="nsew", ipady=10)
