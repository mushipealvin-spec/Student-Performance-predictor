...
Note on the logo:
  If a file named "kalinga_university_logo.png" (or .jpg) is placed in the
  same folder as this script, it will be used automatically. Otherwise a
  generic crest is drawn on the canvas as a placeholder.
"""
...
class ReportCardApp(tk.Tk):
    ...
    def _build_header(self):
        ...
        logo_holder = tk.Frame(inner, bg="#ffffff", width=86, height=86)
        logo_holder.pack()
        logo_holder.pack_propagate(False)
        self._load_logo(logo_holder)

        tk.Label(inner, text="KALINGA UNIVERSITY", font=("Segoe UI", 24, "bold"),
                 bg=BRAND_DARK, fg="#ffffff").pack(pady=(10, 0))
        tk.Label(inner, text="Student Semester Result / Report Card", font=("Segoe UI", 12),
                 bg=BRAND_DARK, fg="#dce7f2").pack()

        band = tk.Frame(header, bg=ACCENT, height=4)
        band.pack(fill="x")

    def _load_logo(self, holder):
        """Uses a real logo image file if present, otherwise draws a placeholder crest."""
        for fname in ("kalinga_university_logo.png", "kalinga_university_logo.jpg",
                      "kalinga-university-logo.png"):
            path = os.path.join(os.path.dirname(os.path.abspath(__file__)), fname)
            if PIL_AVAILABLE and os.path.exists(path):
                try:
                    img = Image.open(path).resize((76, 76))
                    ...
                    pass
        # Placeholder crest (drawn, not a copy of any real logo)
        cv = tk.Canvas(holder, width=76, height=76, bg="#ffffff", highlightthickness=0)
        cv.pack(expand=True)
        cv.create_oval(4, 4, 72, 72, outline=BRAND, width=3)
        cv.create_oval(10, 10, 66, 66, outline=ACCENT, width=1)
        cv.create_text(38, 38, text="KU", font=("Georgia", 20, "bold"), fill=BRAND_DARK)
